export default function LifeStar (graphdata: any, xAxis: number) {
    ...
    console.log(`Life Slope: ${slope}`)
    let wording;
    let description;
    console.log(`Life Now: ${data}`);
    console.log(`Life Next: ${data_1}`);
    if (data >= -100 && data < 0){
        if (slope > 0){
            ...
            +  'เป็นช่วงที่เหมาะกับการตั้งจิต เซ็ตความตั้งใจ ตั้งเป้าหมายใหม่ วางแผนและเตรียมตัวไปสู่สิ่งที่อยากมุ่งหน้าต่อไปในชีวิตของเรา \u003A\u0029';
        } else {
            wording = 'ช่วงทบทวน ไปสู่การเริ่มต้น';
            ...
            + 'และทบทวนเป้าหมาย ความเชื่อ การใช้ชีวิตของเรา สิ่งที่ทำมาอะไรใช่เราอะไรไม่ใช่ อะไรไปต่อ อะไรที่ไม่ตอบโจทย์ชีวิตแล้ว เป็นช่วงที่จะได้กลับมาใกล้ชิดกับตัวเองอีกครั้ง '
        ...
        if (slope > 0){
            ...
            + 'สำหรับใครที่ยังลองหลาย ๆ อย่างอยู่ เป็นช่วงที่เหมาะกับการเริ่มเลือกโฟกัส ใครที่เลือกได้แล้ว พัฒนาจุดแข็งของตัวเอง และเผชิญหน้ากับปัญหาอย่างมั่นคง '
            ...
            + 'อะไรที่ติดค้างมานานก็จะปลดล็อก พบแสงสว่าง สิ่งที่ทำอยู่มีคนมองเห็นคุณค่า   เป็นช่วงที่เหมาะกับการเก็บเกี่ยวผลสำเร็จ '
